...
static inline PositionMaterialNormal computeVertex(EdgeData *edges[12]) {
	...
	return {vertexPosition, cellVertexNormal, 0}; // TODO: material
}

static inline uint32_t convert(uint32_t x, uint32_t y, uint32_t z, uint32_t X, uint32_t Y) {
	return z * Y * X + y * X + x;
}

} // namespace

void extractDualContouringMesh(voxel::RawVolume *volData, const voxel::Palette &palette, const Region &region,
							   Mesh *mesh) {
	...
	const auto lowerCornerY = region.getLowerCorner().z;
	const auto lowerCornerZ = region.getLowerCorner().x;
		...
		for (int32_t cellY = 0; cellY < cellRegionYDimension; cellY++) {
			for (int32_t cellX = 0; cellX < cellRegionXDimension; cellX++) {
				if (cellZ >= 1 && cellY >= 1 && cellX >= 1) {
					...
					{
						const PositionMaterialNormal &vertex = computeVertex(edges);

						VoxelVertex v;
						v.info = 0;
						v.colorIndex = vertex.material;
						...
						cell.vertexIndex = mesh->addVertex(v);
						mesh->setNormal(cell.vertexIndex, vertex.normal);
